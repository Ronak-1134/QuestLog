...
export default function Register() {
  ...
  return (
    <div className="min-h-dvh flex items-center justify-center
                    bg-background noise px-5 py-12">
      ...
      >
        <motion.div variants={staggerItem} className="mb-10 text-center">
          <Link to="/" className="inline-flex items-center gap-2.5">
            <span className="w-7 h-7 rounded bg-foreground flex items-center
                             justify-center text-background font-display font-bold text-xs">
              Q
            </span>
            <span className="font-display font-bold text-lg tracking-tight">QuestLog</span>
          </Link>
        </motion.div>

        <motion.div variants={staggerItem} className="mb-8 text-center">
          <h3 className="text-foreground mb-1">Create your account</h3>
          <p className="text-[var(--color-subtle)] text-sm">Free forever. No credit card.</p>
        </motion.div>

        <motion.div variants={staggerItem} className="flex flex-col gap-2 mb-6">
          <Button variant="ghost" loading={loading.google}
          ...
          <Button variant="ghost" loading={loading.github}
        ...
        <motion.div variants={staggerItem} className="flex items-center gap-3 mb-6">
          ...
          <span className="text-xs text-[var(--color-muted)] font-mono whitespace-nowrap">
            or email
          </span>
          <div className="divider" />
        </motion.div>
        ...
        <motion.div variants={staggerItem} className="flex flex-col gap-4">
          <Input label="Username" type="text" placeholder="yourname"
          ...
          <Button loading={loading.email}
            onClick={() => attempt('email', () => register(form))}
